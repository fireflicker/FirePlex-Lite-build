...
class PlexRepository(private val context: Context) {
    ...
    private val searchIndexCategoryKey = "__fireplex_search_index"
    ...
    suspend fun cachedSearchIndex(limit: Int = 5000): List<PlexMediaItem> = withContext(Dispatchers.IO) {
        mediaCache.firstItems(searchIndexCategoryKey, limit.coerceIn(1, 20_000)).map { it.toMediaItem() }
    }

    suspend fun searchIndexedMedia(query: String, limit: Int = 250): List<PlexMediaItem> = withContext(Dispatchers.IO) {
        val clean = query.trim()
        if (clean.isBlank()) return@withContext cachedSearchIndex(limit)
        val like = "%${clean.replace("%", "").replace("_", "")}%"
        mediaCache.search(searchIndexCategoryKey, like, limit.coerceIn(1, 500)).map { it.toMediaItem() }
    }

    suspend fun rebuildSearchIndex(libraries: List<PlexLibrary>): Int = withContext(Dispatchers.IO) {
        val searchableLibraries = libraries.filter { library ->
            library.type.equals("movie", ignoreCase = true) ||
                library.type.equals("show", ignoreCase = true) ||
                library.type.equals("tv", ignoreCase = true)
        }
        val indexed = mutableListOf<PlexMediaItem>()
        searchableLibraries.forEach { library ->
            runCatching { libraryItems(library) }
                .getOrDefault(emptyList())
                .filter { it.title.isNotBlank() }
                .forEach { item -> indexed += item }
        }
        val distinct = indexed
            .distinctBy { it.ratingKey.ifBlank { it.key.ifBlank { it.title.lowercase() } } }
            .sortedBy { it.title.lowercase() }
        mediaCache.replaceFirstPage(searchIndexCategoryKey, distinct.mapIndexed { index, item -> CachedMediaEntity.from(searchIndexCategoryKey, index, item) })
        distinct.size
    }
    ...
    suspend fun streamMode(): String {
        return context.dataStore.data.first()[streamModeKey]
            ?.takeIf { it == "auto" || it == "direct_play" || it == "direct_stream" || it == "audio_transcode_only" || it == "transcode" }
    ...
    suspend fun migratePlaybackPreferenceToDirectFirst() {
        ...
        context.dataStore.edit { edit ->
            edit[streamModeKey] = "audio_transcode_only"
            edit[directFirstMigrationKey] = true
        }
    }

    suspend fun saveStreamMode(mode: String) {
        val clean = mode.lowercase().takeIf { it == "auto" || it == "direct_play" || it == "direct_stream" || it == "audio_transcode_only" || it == "transcode" } ?: "audio_transcode_only"
        context.dataStore.edit { prefs ->
    ...
    suspend fun streamUrl(item: PlexMediaItem, mode: String = "direct_play"): String = withContext(Dispatchers.IO) {
        ...
        val resolvedMode = if (mode == "auto") "audio_transcode_only" else mode
        ...
        when (resolvedMode) {
            ...
            "direct_stream", "audio_transcode_only" -> plexTranscodeUrl(base, token, item, directStream = true)
            else -> {
